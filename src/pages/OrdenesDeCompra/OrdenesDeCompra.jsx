    ...
    <div className="bg-[#1e1e1e] min-h-screen p-4 sm:p-8 text-white">
      <div className="bg-[#2c2c2c] p-4 sm:p-6 rounded-lg shadow">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
          <h1 className="text-2xl font-bold text-teal-400">Órdenes de Compra</h1>
          <button
          ...
          <table className="w-full text-sm table-auto">
            <thead className="bg-[#1a1a1a] text-teal-400">
              <tr>
                ...
                <th className="px-4 py-2 text-left whitespace-nowrap">Proveedor</th>
            ...
            <tbody>
              {ordenes.length === 0 ? (
                ...
                ordenes.map((orden) => (
                  <tr key={orden.id} className="border-b border-gray-700 hover:bg-[#2a2a2a]">
                    ...
                    <td className="px-4 py-2">
                      <span className={`px-2 py-1 rounded text-sm font-semibold
                        ${
                          orden.estado === 'pendiente' ? 'bg-yellow-400 text-black' :
                          orden.estado === 'finalizado' ? 'bg-green-500 text-white' :
                          orden.estado === 'cancelado' ? 'bg-red-500 text-white' :
      ...
      {ordenAEditar && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70 z-50 px-4">
          <div className="bg-zinc-900 p-6 rounded-xl shadow-lg w-full max-w-sm">
            <h2 className="text-teal-400 text-lg font-bold text-center mb-4">Editar Estado</h2>
            <div className="mb-4">
            ...
            <div className="flex justify-between">
              <button
                onClick={() => actualizarEstado(ordenAEditar.id, ordenAEditar.estado)}
