...
const Switch = ({ checked, onChange }) => (
  <button
    type="button"
    className={`w-12 h-7 flex items-center rounded-full p-1 duration-300 focus:outline-none ${checked ? "bg-[#00d1b2]" : "bg-[#444]"}`}
    onClick={onChange}
  >
    <span
      className={`bg-white w-5 h-5 rounded-full shadow-md transform duration-300 ${checked ? "translate-x-5" : "translate-x-0"}`}
    />
  </button>
);
...
const Modulos = () => {
  ...
  const [sucursalSeleccionada, setSucursalSeleccionada] = useState(() => sessionStorage.getItem("sucursalId") || "");
  const nombreSucursalSeleccionada = sucursales.find(s => s.id === sucursalSeleccionada)?.nombre || "Sucursales";
  ...
  const handleToggleModulo = async (moduloId, checked) => {
    ...
    try {
      if (checked) {
        // Añadir módulo
        const res = await fetch(`${API_URL}/sucursal/${sucursalSeleccionada}/modulos`, {
          method: "POST",
        ...
        // Quitar módulo
        const res = await fetch(`${API_URL}/sucursal/${sucursalSeleccionada}/modulos/${moduloId}`, {
          method: "DELETE",
        });
        if (!res.ok) throw new Error();
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto bg-[#1e1e1e] rounded-xl shadow-lg border border-[#333] text-[#f1f1f1] font-sans">
      ...
      <h1 className="text-4xl font-bold text-[#00d1b2] mb-8">Gestión de Módulos</h1>
      <div className="bg-[#2a2a2a] rounded-xl p-6 shadow-md border border-[#444]">
        {modulos.length === 0 ? (
          <p className="text-center text-gray-400">No hay módulos disponibles.</p>
        ) : (
          <ul className="space-y-4">
            {modulos.map((modulo) => (
              <li key={modulo.id} className="flex items-center justify-between text-xl font-semibold bg-[#1f1f1f] p-4 rounded-lg border border-[#333]">
                <span>{modulo.nombre}</span>
                <Switch
                  checked={modulosSucursal.includes(modulo.id)}
                  onChange={() => handleToggleModulo(modulo.id, !modulosSucursal.includes(modulo.id))}
                />
              </li>
            ))}
          </ul>
