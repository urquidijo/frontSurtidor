...
const InventarioCombustible = () => {
  ...
  const [form, setForm] = useState({
    ...
    id_sucursal: sessionStorage.getItem("sucursalId") || "" // Obtener de sesión si existe
  });
  ...
  const fetchTanques = async () => {
    setLoading(true);
    try {
      console.log("Fetching tanques from:", `${API_URL}/tanques`);
      const res = await fetch(`${API_URL}/tanques`);
  ...
  const handleOpenModal = (isEdit = false, tanque = null) => {
    ...
    if (isEdit && tanque) {
      ...
      setForm({
        ...
        id_sucursal: tanque.id_sucursal || sessionStorage.getItem("sucursalId") || ""
      });
    } else {
      resetForm();
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Validación simple
    if (!form.nombre || !form.capacidad_max || !form.stock || !form.fecha_instalacion || !form.ultima_revision || !form.id_sucursal) {
    ...
    try {
      ...
      const responseText = await res.text(); // Guardar el texto de la respuesta
      ...
      try {
        ...
        console.log(`Tanque ${isEditing ? "actualizado" : "creado"}:`, responseData);
      } catch (e) {
        console.log("Respuesta no es JSON válido, pero la operación fue exitosa");
      }
      
      // Mostrar mensaje de éxito
      alert(`Tanque ${isEditing ? "actualizado" : "creado"} con éxito`);
      ...
      setError(`Error al ${isEditing ? "actualizar" : "crear"} tanque: ${err.message}`);
    } finally {
      setFormSubmitting(false);
    }
  };
  ...
  const handleDeleteTanque = async (id) => {
    if (!window.confirm("¿Estás seguro de que deseas eliminar este tanque? Esta acción no se puede deshacer.")) {
      return;
    }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-[#2a2a2a] rounded-lg p-8 w-full max-w-lg shadow-lg relative border border-[#444]">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                <label className="block mb-1">ID Sucursal *</label>
                <input
                  type="text"
                  name="id_sucursal"
